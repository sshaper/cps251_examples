...
fun TapAnimationExample() {
    ...
    val scale by animateFloatAsState(
        targetValue = if (isPressed) 0.95f else 1f,//is size
...
fun DraggableCardExample() {
    ...
    val animatedOffset by animateOffsetAsState(
        ...
        //used for debugging not for the app to run
        label = "dragOffset"
    )
    Card(
        modifier = Modifier
...
fun LongPressScaleExample() {
    ...
    val scale by animateFloatAsState(
        targetValue = if (isLongPressed) .75f else 1f,
        animationSpec = tween(durationMillis = 200), label = "longPressScale"
    )
    val color by animateColorAsState(
        targetValue = if (isLongPressed) {
            Color(0xFF00FF00) // A custom purple color for long press
        } else {
            MaterialTheme.colorScheme.primary // The default theme color
        },
        animationSpec = tween(durationMillis = 200),
        //label is used dor debudding not needed for the app to run
...
fun InteractiveSliderExample() {
    ...
    // This state will hold the actual width of the slider track in pixels
    ...
    Column(
        ...
        Box(
            ...
            Box(
                modifier = Modifier
                    ...
                    .onSizeChanged { newSize ->
                        // 1. Capture the actual width of the track
                        trackWidth = newSize.width.toFloat()
                    }
                    .pointerInput(trackWidth) { // Re-trigger pointerInput if width changes
                        if (trackWidth == 0f) return@pointerInput // Avoid division by zero
                        detectDragGestures(
                            onDragStart = { isDragging = true },
                            onDragEnd = { isDragging = false },
                            onDrag = { change, dragAmount ->
                                change.consume()
                                // 2. Use the dynamic trackWidth in the calculation
            ...
            Box(
                modifier = Modifier
                    .offset {
                        // 3. Use trackWidth to calculate the thumb's offset
                        val thumbCenterOffset = animatedValue * trackWidth
                        // Center the thumb over the position by subtracting half its width
